...
describe('EquipmentService', () => {
  ...
  it('deve criar solicitação de aprovação ao atualizar equipamento sem perfil administrativo', async () => {
    const equipamentoAtual = { id: 5, secao: { batalhaoId: 2 }, patrimonio: 'EQP-5' };
    const usuario = { id: 10, perfil: PerfilUsuario.USUARIO_BATALHAO } as unknown as UsuarioLogado;
    const dados = { nome: 'Novo Nome' } as any;
    const userFull = { id: 10, perfil: PerfilUsuario.USUARIO_BATALHAO, batalhaoId: 2, secao: { batalhaoId: 2 } };
    ...
    await expect(service.atualizar(5, dados, usuario)).resolves.toEqual({ id: 77 });
    expect(approvalsService.criarSolicitacao).toHaveBeenCalledWith(5, 10, dados, equipamentoAtual);
  });

  it('deve criar solicitação de exclusão quando usuário comum remover equipamento', async () => {
    const equipamentoAtual = { id: 8, secao: { batalhaoId: 3 }, patrimonio: 'EQP-8' };
    const usuario = { id: 11, perfil: PerfilUsuario.USUARIO_BATALHAO } as unknown as UsuarioLogado;

    repository.findEquipamentoById.mockResolvedValue(equipamentoAtual);
    approvalsService.criarSolicitacao.mockResolvedValue({ id: 88 });

    await expect(service.remover(8, usuario)).resolves.toEqual({ id: 88 });
    expect(approvalsService.criarSolicitacao).toHaveBeenCalledWith(8, 11, { _acao: 'DELETE' }, equipamentoAtual);
  });

  it('deve excluir diretamente quando ADMIN_DTEC remover equipamento', async () => {
    const equipamentoAtual = { id: 9, patrimonia: 'EQP-9' };
    const usuario = { id: 1, perfil: PerfilUsuario.ADMIN_DTEC } as unknown as UsuarioLogado;
