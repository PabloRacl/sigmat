...
export class ReportsService {
  ...
  private async buildVisibilityConditions(usuario: any): Promise<any[]> {
    ...
    if (userFull.perfil !== 'ADMIN_DTEC') {
      ...
      } else if (userFull.perfil === PerfilUsuario.USUARIO_BATALHAO && batalhaoId) {
        and.push({ secao: { batalhaoId } });
      } else if (batalhaoId) {
        const or: any[] = [];
        if (secoesIds.length > 0) {
          or.push({ secaoId: { in: secoesIds } });
  ...
  private async buildTransferVisibility(usuario: any): Promise<any> {
    ...
    if (userFull.perfil !== PerfilUsuario.USUARIO_BATALHAO && secoesIds.length > 0) {
  ...
  async logsAuditoria(filters: any = {}, usuario?: any) {
    ...
    if (usuario) {
      ...
      if (userFull && userFull.perfil !== 'ADMIN_DTEC') {
        ...
        } else if (userFull.perfil === PerfilUsuario.USUARIO_BATALHAO && userFull.batalhaoId) {
          where.equipamento = { secao: { batalhaoId: userFull.batalhaoId } };
