...
export class PdfService {
  ...
  async gerarCautelaColetiva(itens: Record<string, any>[]) {
    try {
      const data = {
        ...
        colunas: ['Patrimônio', 'Descrição / Marca / Modelo', 'Nº Série', 'Seção'],
        linhas: itens.map(it => {
          const tipo = (it['tipoEquipamento'] as Record<string, any>)?.['nome'] || '';
          const marca = (it['marca'] as Record<string, any>)?.['nome'] || '';
          const desc = tipo && marca ? `${tipo} - ${marca}` : tipo || marca;
          return [
  ...
  async gerarCautelaCelulares(itens: Record<string, any>[]) {
    try {
      const data = {
        titulo: 'Cautela de Aparelhos Celulares',
        subtitulo: `Gerado em: ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')} | Total: ${itens.length} itens`,
        colunas: ['Patrimônio', 'Marca / Modelo', 'Nº Série', 'IMEI', 'Nº Linha', 'Seção'],
        linhas: itens.map(it => {
          const tipo = (it['tipoEquipamento'] as Record<string, any>)?.['nome'] || '';
          const marca = (it['marca'] as Record<string, any>)?.['nome'] || '';
          const desc = tipo && marca ? `${tipo} - ${marca}` : tipo || marca;
          ...
          if (it['especificacoes']) {
             if (typeof it['especificacoes'] === 'string') {
                try {
                   const spec = JSON.parse(it['especificacoes']);
                   imei = spec.imei || 'N/A';
                   telefone = spec.telefone || 'N/A';
                } catch(e) {}
             } else {
                imei = it['especificacoes'].imei || 'N/A';
                telefone = it['especificacoes'].telefone || 'N/A';
             }
          } else if (it['equipamento']?.['especificacoes']) {
             if (typeof it['equipamento']['especificacoes'] === 'string') {
                try {
                   const spec = JSON.parse(it['equipamento']['especificacoes']);
                   imei = spec.imei || 'N/A';
                   telefone = spec.telefone || 'N/A';
                } catch(e) {}
             } else {
                imei = it['equipamento']['especificacoes'].imei || 'N/A';
                telefone = it['equipamento']['especificacoes'].telefone || 'N/A';
             }
          }
      ...
      if (response) {
        this.downloadBlob(response, `cautela_celulares_${new Date().getTime()}.pdf`);
      }
    } catch (error) {
      console.error('Erro ao gerar cautela de celulares:', error);
    }
  }
