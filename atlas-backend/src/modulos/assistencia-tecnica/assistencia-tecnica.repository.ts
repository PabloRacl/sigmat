...
import { CriarOrdemServicoDto } from './dto/assistencia-tecnica.dto';
...
export class MaintenanceRepository {
  ...
  async getStatusManutencao() {
    return this.prisma.statusEquipamento.findFirst({
      where: { nome: { in: ['MANUTENÇÃO', 'Manutenção', 'MANUTENCAO', 'Em Manutenção', 'EM MANUTENÇÃO'] } },
    });
  }

  async getStatusAtivo() {
    return this.prisma.statusEquipamento.findFirst({
      where: { nome: { in: ['ATIVO', 'Ativo'] } },
    });
  }
