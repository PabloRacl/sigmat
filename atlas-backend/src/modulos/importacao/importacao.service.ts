...
export class ImportacaoService {
  ...
  private async processarEquipamentoIndividual(row: any, patrimonio: string, usuarioId: number) {
    ...
    if (!statusDb) {
      statusDb = await this.prisma.statusEquipamento.findFirst({
        where: { nome: { in: ['DISPONÍVEL', 'Disponível', 'DISPONIVEL'] } },
      });
      if (!statusDb) throw new Error(`Status base 'DISPONÍVEL' não encontrado no banco.`);
    }

    // --- DISPONIBILIDADE ---
    let dispDb = await this.prisma.disponibilidade.findFirst({
      where: { nome: { in: ['ALMOXARIFADO', 'Almoxarifado'] } },
    });
    if (!dispDb) {
